...
const ChatDemo: React.FC = () => {
  ...
  const [input, setInput] = useState('Just finished my 5k new record 24:15');
  ...
  const handleSend = async () => {
    ...
    // 2. Simulate AI Response (No API)
    setTimeout(() => {
      setMessages(prev => [...prev, { role: 'ai', text: "Awesome job! That's 8% faster than your previous best. Be sure to replenish with a glass of water and some fruit. I'll remind you to stretch at 8pm tonight so you aren't sore for your hike with Sarah tomorrow." }]);
      setIsTyping(false);
    }, 1000);
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 scrollbar-hide">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm animate-fade-in ${m.role === 'user'
